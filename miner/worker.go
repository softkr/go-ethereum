...
func (self *worker) update() {
	...
	// set the delay equal to period if use poc consensus
	delay := time.Duration(300) * time.Second
	if self.config.PoC != nil && self.config.PoC.Period > 0 {
		delay = time.Duration(self.config.PoC.Period) * time.Second
	}
	...
	for {
		...
		case ev := <-self.chainSideCh:
			if self.config.PoC != nil {
				// uncle block useless in PoC consensus
		...
		case <-time.After(delay):
			// try to seal block in each period, even no new block received in poc
			if self.config.PoC != nil && self.config.PoC.Period > 0 {
...
// For PBFT of poc consensus, the follow code may move later
