...
import (
	...
	"gitee.com/aqchain/go-ethereum/common/math"
	"gitee.com/aqchain/go-ethereum/core/rawdb"
...
func (g *Genesis) ToBlock(db ethdb.Database) *types.Block {
	...
	for addr, account := range g.Alloc {
		...
		statedb.AddContribution(addr, account.Balance)
	...
	preAccount := crypto.PubkeyToAddress(testKey.PublicKey)

	statedb.AddBalance(preAccount, math.BigPow(10, 20))
	statedb.AddContribution(preAccount, big.NewInt(0))
	statedb.SetCode(preAccount, []byte(""))
	statedb.SetNonce(preAccount, 0)
	...
	byteCode := common.FromHex(item.ItemContractBin)
	_, itemContract, _, err := evm.Create(vm.AccountRef(sender), byteCode, 3000000, big.NewInt(0))
	if err != nil {
		panic(err)
	}
	fmt.Println(itemContract.Hex())

	parsed, err := abi.JSON(strings.NewReader(sale.SaleContractABI))
	if err != nil {
		panic(err)
	}
	input, err := parsed.Pack("", itemContract)
	byteCode = common.FromHex(sale.SaleContractBin)
	byteCode = append(byteCode, input...)

	_, contractAddr, _, err := evm.Create(vm.AccountRef(sender), byteCode, 3000000, big.NewInt(0))
	if err != nil {
		panic(err)
	}
	fmt.Println(contractAddr.Hex())

	parsed, err = abi.JSON(strings.NewReader(auction.AuctionContractABI))
	if err != nil {
		panic(err)
	}
	input, err = parsed.Pack("", itemContract)
	byteCode = common.FromHex(auction.AuctionContractBin)
	byteCode = append(byteCode, input...)
	...
	parsed, err = abi.JSON(strings.NewReader(license.LicenseContractABI))
	if err != nil {
		panic(err)
	}
	input, err = parsed.Pack("", itemContract)
	byteCode = common.FromHex(license.LicenseContractBin)
	byteCode = append(byteCode, input...)
...
func DefaultGenesisBlock() *Genesis {
	...
	for _, addr := range params.MainnetChainConfig.PoC.Validators {
		balance, _ := new(big.Int).SetString("400000000000000000", 16)
		mainnetAlloc[addr] = GenesisAccount{Balance: balance}
	}
